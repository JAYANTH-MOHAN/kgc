...
llm = LLM(model="meta-llama/Meta-Llama-3-8B-Instruct",gpu_memory_utilization=1,max_model_len=4096,tensor_parallel_size=2)
...
system_prompt="You are an expert in annotating keyphrases for a scientific document from their title and abstract."
...
def keyphrase_generator(title,abstract):
    ...
    prompt_try_1= f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n{system_prompt}<|eot_id|><|start_header_id|>user<|end_header_id|>\n{user_prompt}\n**Title:** \"{title}\"\n\n**Abstract:** \"{abstract}\"\n\n **Instructions:** \n{INS1} <|eot_id|><|start_header_id|>assistant<|end_header_id|>\n"


    request = prompt_try_1
...
for i in range(len(total_data)):
    ...
    all_pred_keyphrases.append(last_part.replace('"', '').replace("'", "").split(', '))
...
with open("kpp_keygen.txt", "w") as file:
    file.write(output_text)



import os
file_path = "Llama_KP20K_latest.txt"
