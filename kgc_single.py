...
llm = LLM(model="microsoft/Phi-3-mini-128k-instruct",gpu_memory_utilization=0.95,max_model_len=4096)  
...
# Initialize lists to store keyphrases and their KPP-s values
tokenized_keyphrases = []
keyphrase_kpp_s_values = []
keyphrases = []
word_counts = []

# Your keyphrase generation function

generated_keyphrases = keyphrase_generator(total_data[0]['title'],  total_data[0]['abstract'])
...
# Loop through each keyphrase
for keyphrase in keyphrases:
    temp_keyphrase_tokens = keyphrase.split()
    token_info_list = []
    word_count = len(temp_keyphrase_tokens)

    # Loop through log probabilities and extract token information for current keyphrase
    for token_probs in log_probs:
        for token_id, logprob_info in token_probs.items():
            decoded_token = logprob_info.decoded_token  # Access attribute directly
            if decoded_token in temp_keyphrase_tokens:
                log_prob = logprob_info.logprob  # Access attribute directly
                token_info_list.append({'token_id': token_id, 'log_prob': log_prob, 'decoded_token': decoded_token})

    # Calculate KPP-s for the current keyphrase
...
gen_list = [preprocess_abstract(f"{item.strip()}") for item in keyphrases]
print(gen_list)
print()
...
def jumbled_string_match(list1, list2):
    """
    Compare two lists of strings for exact matches, considering that the strings can be jumbled.
    """
    # Convert lists to sets for efficient comparison
    set1 = set(list1)
    set2 = set(list2)
    matching_labels = set1.intersection(set2)
    ...
    return accuracy, f1,matching_labels


accuracy, f1,matching_labels = jumbled_string_match(gen_list, preprocessed_keyphrases[0])

print("Accuracy:", accuracy)
print("F1 Score:", f1)
print("matches:", matching_labels)
...
def process_abstract(abstract_index):
    abstract = abstracts[abstract_index]
    processed_src = preprocess_abstract(remove_numbers_and_dots_from_string(abstract))
...
file_path = os.path.join(output_dir, "Phi_KP20K_singlegpu.txt")
...
for idx in range(len(total_data)):
    ...
    # Append present and absent keyphrases to the ordered lists
